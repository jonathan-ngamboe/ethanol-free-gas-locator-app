...
export default function ListScreen({ navigation }) {
  ...
  const { favoriteStations, searchHistory } = useStation();
  ...
  const renderStationCount = () => {
    ...
    if (count === 0 || !count)
      return `No stations in your ${pageTitle?.toLowerCase()}`;
  ...
  return (
    <View style={styles.container}>
      <List.Section style={localStyles.contentContainer}>
        {/* Header */}
        <List.Section
          style={[localStyles.header, styles.contentPaddingHorizontal]}
        >
          <List.Item
            ...
            left={() => (
              <List.Icon
                icon={
                  stationList?.length > 10 ? "gesture-swipe-vertical" : pageIcon
                }
          ...
          {stationList?.length > 0 && (
            <>
              <Text
                style={[
                  styles.listSection,
                  { color: theme.colors.outline, fontWeight: "bold" },
                ]}
              >
                View mode
              </Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <MemoizedStationCard
                  ...
                  showShadow={false}
                  onPressCard={() =>
                    navigation.navigate("StationDetails", { station: item })
                  }
